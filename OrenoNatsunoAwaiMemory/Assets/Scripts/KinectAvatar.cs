...
public class KinectAvatar : MonoBehaviour
{
    ...
    public bool UseGuitaristGen = true;

    public BodySourceManager _BodyManager;
    public GameObject _UnityChan;
    public float unityChanPosY;
    //GuitarintGenをここにいれる
    ...
    void Start()
    {
        //ゲームスタート時に生成されたオブジェクトを登録
        if(UseGuitaristGen) _UnityChan = target.GetComponent<GuitaristGen>().GetGuitarist();
    ...
    void Update()
    {
        ...
        Quaternion HandLeft = new Quaternion();
        Quaternion ElbowRight;
        Quaternion WristRight;
        Quaternion HandRight = new Quaternion();
        Quaternion KneeLeft;
        ...
        if(UseGuitaristGen) _UnityChan.transform.position = new Vector3(-pos.X, unityChanPosY, -pos.Z);
    }
}